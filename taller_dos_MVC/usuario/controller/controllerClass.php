  ...
  public function create() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      if ($_POST['id']){
        ...
        $rsp = modelClass::putNewUser($_POST['id'], $_POST['usuario'], $_POST['password'], $_POST['activado']);
        if ($rsp === true) {
      ...
      if (is_array($certificate)) {
        if (count($certificate) > 0) {
          ...
          if (is_array($data)) {
            if (count($data) > 0) {
              $args['id'] = $data[0]['id'];
              $args['usuario'] = $data[0]['usuario'];
              $args['password'] = $data[0]['password'];
              $args['activado'] = $data[0]['activado'];
            }
          } else {
            $args['error'] = $data;
